document.addEventListener('DOMContentLoaded', async () => {
            if (window.ethereum) {
                ...
                async function updateStats() {
                    ...
                    const userDeposits = await contract.methods.users(userAccount).call().then(user => user.deposits);
                    const userWithdrawals = await contract.methods.users(userAccount).call().then(user => user.withdrawn);
                    const userDividendsToday = await contract.methods.getUserAvailableDividends(userAccount).call();
                    const userTotalDividends = await contract.methods.users(userAccount).call().then(user => user.dividendsClaimed);
